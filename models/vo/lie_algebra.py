...
# code from https://github.com/mbrossar/denoise-imu-gyro
class SO3_CUDA(object):
    def __init__(self, device):
        #  tolerance criterion
        self.TOL = 1e-8
        self.Id = torch.eye(3).float().to(device)
        self.dId = torch.eye(3).double().to(device)
    ...
    def to_quaternion(cls, Rots, ordering='wxyz'):
        ...
        if near_zero_mask.sum() > 0:
            ...
            cond1_inds = cond1_mask.nonzero(as_tuple=False)
            ...
            cond2_inds = cond2_mask.nonzero(as_tuple=False)
...
class SO3_CPU(SO3_CUDA):
    def __init__(self):
        #  tolerance criterion
        self.TOL = 1e-8
        self.Id = torch.eye(3).float()
        self.dId = torch.eye(3).double()
