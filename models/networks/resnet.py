...
from models.networks.running_mean_and_var import RunningMeanAndVar
import torch.nn.functional as F
import numpy as np
from models.common.obs_transforms import Resizer
...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, observations):
        ...
        if self._n_input_rgb > 0:
            ...
            cnn_inputs.append(rgb_observations)

        if self._n_input_depth > 0:
            depth_observations = observations["depth"]
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            depth_observations = depth_observations.permute(0, 3, 1, 2).contiguous()
            cnn_inputs.append(depth_observations)

        if self.obs_transform:
            cnn_inputs = [self.obs_transform(inp) for inp in cnn_inputs]

        x = torch.cat(cnn_inputs, dim=1)
        x = F.avg_pool2d(x, 2)
...
class VisualEmbeding(nn.Module):
    def __init__(self, visual_encoder_shape, outputs_channels):
        super().__init__()

        self.fc1 = nn.Linear(np.prod(visual_encoder_shape), outputs_channels)
        self.relu = nn.ReLU(True)
