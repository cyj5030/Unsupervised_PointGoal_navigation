...
class VectorEnv:
    ...
    def _worker_env(
        ...
        follower = ShortestPathFollower(env._env.sim, env_fn_args[0].TASK_CONFIG.TASK.SUCCESS_DISTANCE, False)
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                elif command == ACTION_COMMAND:
                    target_pose = data
                    if target_pose is None:
                        target_pose = env.current_episode.goals[0].position
                    act = follower.get_next_action(target_pose)
                    connection_write_fn(act)

                elif command == STATE_COMMAND:
                    result_agent_state = env._env._sim.get_agent_state()
                    connection_write_fn(result_agent_state)
